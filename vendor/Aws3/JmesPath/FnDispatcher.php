...
class FnDispatcher
{
    ...
    private function fn_avg(array $args)
    {
        ...
        $sum = $this->reduce('avg:0', $args[0], ['number'], function ($a, $b) {
            return \BaghelSoft\S3Uploads\JmesPath\Utils::add($a, $b);
        });
    ...
    private function fn_sort(array $args)
    {
        ...
        $valid = ['string', 'number'];
        return \BaghelSoft\S3Uploads\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($valid) {
    ...
    private function fn_sort_by(array $args)
    {
        ...
        $expr = $args[1];
        $valid = ['string', 'number'];
        return \BaghelSoft\S3Uploads\JmesPath\Utils::stableSort($args[0], function ($a, $b) use($expr, $valid) {
            $va = $expr($a);
            $vb = $expr($b);
    ...
    private function fn_type(array $args)
    {
        ...
        return \BaghelSoft\S3Uploads\JmesPath\Utils::type($args[0]);
    }
    ...
    private function fn_to_number(array $args)
    {
        ...
        $value = $args[0];
        $type = \BaghelSoft\S3Uploads\JmesPath\Utils::type($value);
        if ($type == 'number') {
            return $value;
    ...
    private function fn_to_array(array $args)
    {
        ...
        return \BaghelSoft\S3Uploads\JmesPath\Utils::isArray($args[0]) ? $args[0] : [$args[0]];
    }
    ...
    {
        if ($types[0] == 'any' || in_array(\BaghelSoft\S3Uploads\JmesPath\Utils::type($value), $types) || $value === [] && in_array('object', $types)) {
            return;
        }
        $msg = 'must be one of the following types: ' . implode(', ', $types) . '. ' . \BaghelSoft\S3Uploads\JmesPath\Utils::type($value) . ' found';
    ...
    {
        $ta = \BaghelSoft\S3Uploads\JmesPath\Utils::type($a);
        $tb = \BaghelSoft\S3Uploads\JmesPath\Utils::type($b);
        if ($ta !== $tb) {
