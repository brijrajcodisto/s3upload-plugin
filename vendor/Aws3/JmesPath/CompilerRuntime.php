...
class CompilerRuntime
{
     ...
     */
    public function __construct($dir = null, \BaghelSoft\S3Uploads\JmesPath\Parser $parser = null)
    {
        $this->parser = $parser ?: new \BaghelSoft\S3Uploads\JmesPath\Parser();
        $this->compiler = new \BaghelSoft\S3Uploads\JmesPath\TreeCompiler();
        $dir = $dir ?: sys_get_temp_dir();
        if (!is_dir($dir) && !mkdir($dir, 0755, true)) {
            throw new \RuntimeException("Unable to create cache directory: {$dir}");
        }
        $this->cacheDir = realpath($dir);
        $this->interpreter = new \BaghelSoft\S3Uploads\JmesPath\TreeInterpreter();
    }
    /**
