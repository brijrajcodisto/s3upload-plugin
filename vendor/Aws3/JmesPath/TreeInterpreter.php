...
class TreeInterpreter
{
    ...
    {
        $this->fnDispatcher = $fnDispatcher ?: \BaghelSoft\S3Uploads\JmesPath\FnDispatcher::getInstance();
    }
    /**
    ...
    private function dispatch(array $node, $value)
    {
        ...
        switch ($node['type']) {
            ...
            case 'index':
                if (!\BaghelSoft\S3Uploads\JmesPath\Utils::isArray($value)) {
                    return null;
                }
            ...
            case 'projection':
                ...
                switch ($node['from']) {
                    case 'object':
                        if (!\BaghelSoft\S3Uploads\JmesPath\Utils::isObject($left)) {
                            return null;
                        }
                        break;
                    case 'array':
                        if (!\BaghelSoft\S3Uploads\JmesPath\Utils::isArray($left)) {
                            return null;
                        }
                        break;
                    default:
            ...
            case 'or':
                ...
                return \BaghelSoft\S3Uploads\JmesPath\Utils::isTruthy($result) ? $result : $this->dispatch($node['children'][1], $value);
            case 'and':
                $result = $this->dispatch($node['children'][0], $value);
                return \BaghelSoft\S3Uploads\JmesPath\Utils::isTruthy($result) ? $this->dispatch($node['children'][1], $value) : $result;
            case 'not':
                return !\BaghelSoft\S3Uploads\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value));
            case 'pipe':
                return $this->dispatch($node['children'][1], $this->dispatch($node['children'][0], $value));
            ...
            case 'comparator':
                ...
                if ($node['value'] == '==') {
                    return \BaghelSoft\S3Uploads\JmesPath\Utils::isEqual($left, $right);
                } elseif ($node['value'] == '!=') {
                    return !\BaghelSoft\S3Uploads\JmesPath\Utils::isEqual($left, $right);
                } else {
            ...
            case 'condition':
                return \BaghelSoft\S3Uploads\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value)) ? $this->dispatch($node['children'][1], $value) : null;
            case 'function':
                $args = [];
            ...
            case 'slice':
                return is_string($value) || \BaghelSoft\S3Uploads\JmesPath\Utils::isArray($value) ? \BaghelSoft\S3Uploads\JmesPath\Utils::slice($value, $node['value'][0], $node['value'][1], $node['value'][2]) : null;
            case 'expref':
