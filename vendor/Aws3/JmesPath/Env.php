...
final class Env
{
    ...
    public static function search($expression, $data)
    {
        static $runtime;
        if (!$runtime) {
            $runtime = \BaghelSoft\S3Uploads\JmesPath\Env::createRuntime();
        }
    ...
    public static function createRuntime()
    {
        switch ($compileDir = self::getEnvVariable(self::COMPILE_DIR)) {
            case false:
                return new \BaghelSoft\S3Uploads\JmesPath\AstRuntime();
            case 'on':
                return new \BaghelSoft\S3Uploads\JmesPath\CompilerRuntime();
            default:
                return new \BaghelSoft\S3Uploads\JmesPath\CompilerRuntime($compileDir);
        }
    }
    /**
