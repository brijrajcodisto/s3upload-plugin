 ...
 */
class EachPromise implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    private $pending = [];
    /** @var \Iterator|null */
    private $iterable;
    ...
    {
        $this->iterable = \BaghelSoft\S3Uploads\GuzzleHttp\Promise\Create::iterFor($iterable);
    ...
    {
        $this->mutex = false;
        $this->aggregate = new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\Promise(function () {
            reset($this->pending);
            ...
            while ($promise = current($this->pending)) {
                next($this->pending);
                $promise->wait();
                if (\BaghelSoft\S3Uploads\GuzzleHttp\Promise\Is::settled($this->aggregate)) {
                    return;
                }
            }
        });
    ...
    private function addPending()
    {
        ...
        $promise = \BaghelSoft\S3Uploads\GuzzleHttp\Promise\Create::promiseFor($this->iterable->current());
