...
namespace BaghelSoft\S3Uploads\Aws\Credentials;

use Aws;
use BaghelSoft\S3Uploads\Aws\Api\DateTimeResult;
use BaghelSoft\S3Uploads\Aws\CacheInterface;
use BaghelSoft\S3Uploads\Aws\Exception\CredentialsException;
use BaghelSoft\S3Uploads\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        $shouldUseEcsCredentialsProvider = getenv(\BaghelSoft\S3Uploads\Aws\Credentials\EcsCredentialProvider::ENV_URI);
        // getenv() is not thread safe - fall back to $_SERVER
        if ($shouldUseEcsCredentialsProvider === false) {
            $shouldUseEcsCredentialsProvider = isset($_SERVER[\BaghelSoft\S3Uploads\Aws\Credentials\EcsCredentialProvider::ENV_URI]) ? $_SERVER[\BaghelSoft\S3Uploads\Aws\Credentials\EcsCredentialProvider::ENV_URI] : false;
        }
     ...
     * @return callable
     */
    public static function fromCredentials(\BaghelSoft\S3Uploads\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\BaghelSoft\S3Uploads\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \BaghelSoft\S3Uploads\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof CredentialsInterface && !$found->isExpired()) {
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\BaghelSoft\S3Uploads\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($key, $secret, getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \BaghelSoft\S3Uploads\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    public static function sso($ssoProfileName, $filename = null, $config = [])
    {
        ...
        return function () use($ssoProfileName, $filename, $config) {
            ...
            try {
                $expiration = (new \BaghelSoft\S3Uploads\Aws\Api\DateTimeResult($tokenData['expiresAt']))->getTimestamp();
            } catch (\Exception $e) {
            ...
            if (empty($config['ssoClient'])) {
                $ssoClient = new \BaghelSoft\S3Uploads\Aws\SSO\SSOClient(['region' => $ssoProfile['sso_region'], 'version' => '2019-06-10', 'credentials' => false]);
            } else {
            ...
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($ssoCredentials['accessKeyId'], $ssoCredentials['secretAccessKey'], $ssoCredentials['sessionToken'], $expiration));
        };
    }
    /**
    ...
    {
        return new \BaghelSoft\S3Uploads\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \BaghelSoft\S3Uploads\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \BaghelSoft\S3Uploads\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \BaghelSoft\S3Uploads\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \BaghelSoft\S3Uploads\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
            if ($data === false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \BaghelSoft\S3Uploads\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \BaghelSoft\S3Uploads\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient, $config = [])
    {
        ...
        if (empty($stsClient)) {
            ...
            if (!empty($roleProfile['source_profile'])) {
                $sourceCredentials = call_user_func(\BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            } else {
                $sourceCredentials = self::getCredentialsFromSource($profileName, $filename);
            }
            $stsClient = new \BaghelSoft\S3Uploads\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $credentials = $stsClient->createCredentials($result);
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($credentials);
    }
    /**
    ...
    {
        $profileData = \BaghelSoft\S3Uploads\Aws\parse_ini_file($filename, true, INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \BaghelSoft\S3Uploads\Aws\parse_ini_file($configFilename, true, INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (file_exists($credFile)) {
            $profiles = \BaghelSoft\S3Uploads\Aws\parse_ini_file($credFile, true, INI_SCANNER_RAW);
        }
        if (file_exists($configFile)) {
            $configProfileData = \BaghelSoft\S3Uploads\Aws\parse_ini_file($configFile, true, INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = str_replace('profile ', '', $name);
    ...
    public static function getCredentialsFromSource($profileName = '', $filename = '', $config = [])
    {
        ...
        switch ($credentialSource) {
            ...
            case 'EcsContainer':
                ...
                break;
            default:
                throw new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException("Invalid credential_source found in config file: {$credentialSource}. Valid inputs " . "include Environment, Ec2InstanceMetadata, and EcsContainer.");
        }
        ...
        return function () use($credentialsResult) {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($credentialsResult);
        };
    }
    private static function reject($msg)
    {
        return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise(new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException($msg));
    }
    /**
     * @param $filename
     * @return string
