...
use BaghelSoft\S3Uploads\Aws\Exception\InvalidJsonException;
use BaghelSoft\S3Uploads\Aws\Sdk;
use BaghelSoft\S3Uploads\GuzzleHttp\Exception\TransferException;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->attempts = 0;
        $this->client = isset($config['client']) ? $config['client'] : \BaghelSoft\S3Uploads\Aws\default_http_handler();
    }
    /**
    ...
    {
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () {
            ...
            (yield new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], strtotime($result['Expiration'])));
        });
    }
    /**
     * @param string $url
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (strcasecmp($disabled, 'true') === 0) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \BaghelSoft\S3Uploads\Aws\Sdk::VERSION;
        if (defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \BaghelSoft\S3Uploads\Aws\default_user_agent();
        $request = $request->withHeader('User-Agent', $userAgent);
        foreach ($headers as $key => $value) {
            $request = $request->withHeader($key, $value);
        }
        return $fn($request, ['timeout' => $this->timeout])->then(function (\BaghelSoft\S3Uploads\Psr\Http\Message\ResponseInterface $response) {
            ...
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (json_last_error() > 0) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\InvalidJsonException();
        }
        if ($result['Code'] !== 'Success') {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
