...
use Iterator;
use BaghelSoft\S3Uploads\Aws\Exception\EventStreamDataException;
...
class EventParsingIterator implements \Iterator
{
    ...
    /** @var AbstractParser */
    private $parser;
    public function __construct(\BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface $stream, \BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, \BaghelSoft\S3Uploads\Aws\Api\Parser\AbstractParser $parser)
    {
        $this->decodingIterator = new \BaghelSoft\S3Uploads\Aws\Api\Parser\DecodingEventStreamIterator($stream);
        $this->shape = $shape;
        $this->parser = $parser;
    }
    ...
    private function parseEvent(array $event)
    {
        if (!empty($event['headers'][':message-type'])) {
            ...
            if ($event['headers'][':message-type'] !== 'event') {
                throw new \BaghelSoft\S3Uploads\Aws\Api\Parser\Exception\ParserException('Failed to parse unknown message type.');
            }
        }
        if (empty($event['headers'][':event-type'])) {
            throw new \BaghelSoft\S3Uploads\Aws\Api\Parser\Exception\ParserException('Failed to parse without event type.');
        }
    ...
    {
        throw new \BaghelSoft\S3Uploads\Aws\Exception\EventStreamDataException($event['headers'][':error-code'], $event['headers'][':error-message']);
    }
}
