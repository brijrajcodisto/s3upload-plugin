...
use BaghelSoft\S3Uploads\Aws\Api\Serializer\QuerySerializer;
use BaghelSoft\S3Uploads\Aws\Api\Serializer\Ec2ParamBuilder;
use BaghelSoft\S3Uploads\Aws\Api\Parser\QueryParser;
/**
 * Represents a web service API model.
 */
class Service extends \BaghelSoft\S3Uploads\Aws\Api\AbstractModel
{
    /** @var callable */
    private $apiProvider;
    /** @var string */
    ...
    public function __construct(array $definition, callable $provider)
    {
        ...
        parent::__construct($definition, new \BaghelSoft\S3Uploads\Aws\Api\ShapeMap($definition['shapes']));
     ...
     */
    public static function createSerializer(\BaghelSoft\S3Uploads\Aws\Api\Service $api, $endpoint)
    {
        static $mapping = ['json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Serializer\\JsonRpcSerializer', 'query' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Serializer\\QuerySerializer', 'rest-json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Serializer\\RestJsonSerializer', 'rest-xml' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Serializer\\RestXmlSerializer'];
        $proto = $api->getProtocol();
        if (isset($mapping[$proto])) {
            return new $mapping[$proto]($api, $endpoint);
        }
        if ($proto == 'ec2') {
            return new \BaghelSoft\S3Uploads\Aws\Api\Serializer\QuerySerializer($api, $endpoint, new \BaghelSoft\S3Uploads\Aws\Api\Serializer\Ec2ParamBuilder());
        }
     ...
     * @throws \UnexpectedValueException
     */
    public static function createErrorParser($protocol, \BaghelSoft\S3Uploads\Aws\Api\Service $api = null)
    {
        static $mapping = ['json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\ErrorParser\\JsonRpcErrorParser', 'query' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\ErrorParser\\XmlErrorParser', 'rest-json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\ErrorParser\\RestJsonErrorParser', 'rest-xml' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\ErrorParser\\XmlErrorParser', 'ec2' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\ErrorParser\\XmlErrorParser'];
     ...
     * @throws \UnexpectedValueException
     */
    public static function createParser(\BaghelSoft\S3Uploads\Aws\Api\Service $api)
    {
        static $mapping = ['json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Parser\\JsonRpcParser', 'query' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Parser\\QueryParser', 'rest-json' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Parser\\RestJsonParser', 'rest-xml' => 'BaghelSoft\\S3Uploads\\Aws\\Api\\Parser\\RestXmlParser'];
        $proto = $api->getProtocol();
        if (isset($mapping[$proto])) {
            return new $mapping[$proto]($api);
        }
        if ($proto == 'ec2') {
            return new \BaghelSoft\S3Uploads\Aws\Api\Parser\QueryParser($api, null, false);
        }
        throw new \UnexpectedValueException('Unknown protocol: ' . $api->getProtocol());
    }
    /**
    ...
    public function getOperation($name)
    {
        if (!isset($this->operations[$name])) {
            ...
            $this->operations[$name] = new \BaghelSoft\S3Uploads\Aws\Api\Operation($this->definition['operations'][$name], $this->shapeMap);
        }
    ...
    public function getErrorShapes()
    {
        ...
        foreach ($this->definition['shapes'] as $name => $definition) {
            if (!empty($definition['exception'])) {
                ...
                $result[] = new \BaghelSoft\S3Uploads\Aws\Api\StructureShape($definition, $this->getShapeMap());
            }
        }
        return $result;
    }
