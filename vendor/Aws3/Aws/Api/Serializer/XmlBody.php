...
class XmlBody
{
     ...
     * @return string
     */
    public function build(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, array $args)
    {
        $xml = new \XMLWriter();
        $xml->openMemory();
    ...
    private function startElement(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, \XMLWriter $xml)
    {
    ...
    private function format(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
    ...
    private function defaultShape(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->text($value);
        $xml->endElement();
    }
    private function add_structure(\BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, $name, array $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
    ...
    private function getStructureMembers(\BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, array $value)
    {
        $members = [];
    ...
    private function add_list(\BaghelSoft\S3Uploads\Aws\Api\ListShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_map(\BaghelSoft\S3Uploads\Aws\Api\MapShape $shape, $name, array $value, \XMLWriter $xml)
    {
    ...
    private function add_blob(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw(base64_encode($value));
        $xml->endElement();
    }
    private function add_timestamp(\BaghelSoft\S3Uploads\Aws\Api\TimestampShape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $xml->writeRaw(\BaghelSoft\S3Uploads\Aws\Api\TimestampShape::format($value, $timestampFormat));
        $xml->endElement();
    }
    private function add_boolean(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
        $this->startElement($shape, $name, $xml);
        $xml->writeRaw($value ? 'true' : 'false');
        $xml->endElement();
    }
    private function add_string(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $name, $value, \XMLWriter $xml)
    {
