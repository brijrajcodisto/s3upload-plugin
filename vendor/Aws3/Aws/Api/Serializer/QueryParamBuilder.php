...
class QueryParamBuilder
{
    ...
    public function __invoke(\BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, array $params)
    {
        if (!$this->methods) {
    ...
    protected function format(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
    ...
    protected function format_structure(\BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, array $value, $prefix, &$query)
    {
        if ($prefix) {
            $prefix .= '.';
        }
    ...
    protected function format_map(\BaghelSoft\S3Uploads\Aws\Api\MapShape $shape, array $value, $prefix, array &$query)
    {
        $vals = $shape->getValue();
        $keys = $shape->getKey();
        if (!$this->isFlat($shape)) {
    ...
    protected function format_blob(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
        $query[$prefix] = base64_encode($value);
    }
    protected function format_timestamp(\BaghelSoft\S3Uploads\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query)
    {
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $query[$prefix] = \BaghelSoft\S3Uploads\Aws\Api\TimestampShape::format($value, $timestampFormat);
    }
    protected function format_boolean(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
