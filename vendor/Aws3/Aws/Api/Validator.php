...
class Validator
{
     ...
     */
    public function validate($name, \BaghelSoft\S3Uploads\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\BaghelSoft\S3Uploads\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\BaghelSoft\S3Uploads\Aws\Api\ListShape $shape, $value)
    {
        if (!is_array($value)) {
            $this->addError('must be an array. Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\BaghelSoft\S3Uploads\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => true, 'integer' => true, 'double' => true, 'resource' => true];
        $type = gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'BaghelSoft\\S3Uploads\\GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value)
    {
        if (!is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value)
    {
        if (!is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
        }
    }
    private function check_string(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\BaghelSoft\S3Uploads\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        ...
        if (!$isAssociative) {
            $this->addError('must be an associative array. Found ' . \BaghelSoft\S3Uploads\Aws\describe_type($value));
            return false;
        }
        return true;
    }
