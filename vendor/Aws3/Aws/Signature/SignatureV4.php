...
use BaghelSoft\S3Uploads\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \BaghelSoft\S3Uploads\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
     ...
     */
    public static function convertPostToGet(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, $additionalQueryParams = "")
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, \BaghelSoft\S3Uploads\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
