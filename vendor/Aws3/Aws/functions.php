...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \BaghelSoft\S3Uploads\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \BaghelSoft\S3Uploads\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \BaghelSoft\S3Uploads\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \BaghelSoft\S3Uploads\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (defined('BaghelSoft\\S3Uploads\\GuzzleHttp\\ClientInterface::VERSION')) {
        $version = (string) \BaghelSoft\S3Uploads\GuzzleHttp\ClientInterface::VERSION;
        if ($version[0] === '6') {
            return $cache = 6;
        }
        if ($version[0] === '5') {
    ...
    } elseif (defined('BaghelSoft\\S3Uploads\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
        return $cache = \BaghelSoft\S3Uploads\GuzzleHttp\ClientInterface::MAJOR_VERSION;
    }
 ...
 * @throws \RuntimeException
 */
function serialize(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\BaghelSoft\S3Uploads\Aws\CommandInterface $_, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\FulfilledPromise(new \BaghelSoft\S3Uploads\Aws\Result([]));
    });
