...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'BaghelSoft\\S3Uploads\\Aws\\Exception\\AwsException', $collectStats = false)
    {
    ...
    public function __invoke(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\BaghelSoft\S3Uploads\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, \BaghelSoft\S3Uploads\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \BaghelSoft\S3Uploads\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, \BaghelSoft\S3Uploads\Aws\CommandInterface $command, array $stats)
    {
