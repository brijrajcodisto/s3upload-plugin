 ...
 */
class CommandPool implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $this->each = new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
