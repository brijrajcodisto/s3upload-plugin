 ...
 * @internal
 */
class ApiCallAttemptMonitoringMiddleware extends \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    private static function getResultAttemptLatency(\BaghelSoft\S3Uploads\Aws\ResultInterface $result)
    {
    ...
    private static function getResultDestinationIp(\BaghelSoft\S3Uploads\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultDnsLatency(\BaghelSoft\S3Uploads\Aws\ResultInterface $result)
    {
        if (isset($result['@metadata']['transferStats']['http'])) {
            $attempt = end($result['@metadata']['transferStats']['http']);
    ...
    private static function getResultHttpStatusCode(\BaghelSoft\S3Uploads\Aws\ResultInterface $result)
    {
        return $result['@metadata']['statusCode'];
    }
    private static function getAwsExceptionAttemptLatency(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
    ...
    private static function getAwsExceptionErrorCode(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorCode();
    }
    private static function getAwsExceptionMessage(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
        return $e->getAwsErrorMessage();
    }
    private static function getAwsExceptionDestinationIp(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['primary_ip'])) {
            return $attempt['primary_ip'];
    ...
    private static function getAwsExceptionDnsLatency(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
        $attempt = $e->getTransferInfo();
        if (isset($attempt['namelookup_time'])) {
            return (int) floor($attempt['namelookup_time'] * 1000);
    ...
    private static function getAwsExceptionHttpStatusCode(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e)
    {
