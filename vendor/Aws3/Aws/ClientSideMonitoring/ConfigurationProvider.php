...
use BaghelSoft\S3Uploads\Aws\AbstractConfigurationProvider;
use BaghelSoft\S3Uploads\Aws\CacheInterface;
use BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Exception\ConfigurationException;
use BaghelSoft\S3Uploads\Aws\ConfigurationProviderInterface;
 ...
 * </code>
 */
class ConfigurationProvider extends \BaghelSoft\S3Uploads\Aws\AbstractConfigurationProvider implements \BaghelSoft\S3Uploads\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationInterface::class;
    protected static $exceptionClass = \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if ($enabled !== false) {
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration($enabled, getenv(self::ENV_HOST) ?: self::DEFAULT_HOST, getenv(self::ENV_PORT) ?: self::DEFAULT_PORT, getenv(self::ENV_CLIENT_ID) ?: self::DEFAULT_CLIENT_ID));
            }
    ...
    {
        return function () {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration(self::DEFAULT_ENABLED, self::DEFAULT_HOST, self::DEFAULT_PORT, self::DEFAULT_CLIENT_ID));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \BaghelSoft\S3Uploads\Aws\parse_ini_file($filename, true);
            if ($data === false) {
            ...
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration($data[$profile]['csm_enabled'], $data[$profile]['csm_host'], $data[$profile]['csm_port'], $data[$profile]['csm_client_id']));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof ConfigurationInterface) {
            ...
            return new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration($config['enabled'], $host, $port, $client_id);
        }
