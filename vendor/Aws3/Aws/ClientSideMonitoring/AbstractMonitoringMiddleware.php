 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\BaghelSoft\S3Uploads\Aws\ResultInterface $result, $headerName)
    {
    ...
    public function __invoke(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) floor(microtime(true) * 1000), 'UserAgent' => substr($request->getHeaderLine('User-Agent') . ' ' . \BaghelSoft\S3Uploads\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
     ...
     * @return array
     */
    protected function populateRequestEventData(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, array $event)
    {
    ...
    private function unwrappedOptions()
    {
        if (!$this->options instanceof ConfigurationInterface) {
            try {
                $this->options = \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::unwrap($this->options);
            } catch (\Exception $e) {
                // Errors unwrapping CSM config defaults to disabling it
                $this->options = new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration(false, \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT);
            }
        }
        return $this->options;
    }
