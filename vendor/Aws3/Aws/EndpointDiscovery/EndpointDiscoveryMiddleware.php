...
namespace BaghelSoft\S3Uploads\Aws\EndpointDiscovery;
...
class EndpointDiscoveryMiddleware
{
    ...
    public function __construct(callable $handler, \BaghelSoft\S3Uploads\Aws\AwsClient $client, array $args, $config)
    {
    ...
    public function __invoke(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (isset($op['endpointdiscovery'])) {
            $config = \BaghelSoft\S3Uploads\Aws\EndpointDiscovery\ConfigurationProvider::unwrap($this->config);
            $isRequired = !empty($op['endpointdiscovery']['required']);
            if ($isRequired && !$config->isEnabled()) {
                throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException('This operation ' . 'requires the use of endpoint discovery, but this has ' . 'been disabled in the configuration. Enable endpoint ' . 'discovery or use a different operation.');
            }
            // Continue only if enabled by config
            if ($config->isEnabled()) {
                if (isset($op['endpointoperation'])) {
                    throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException('This operation is ' . 'contradictorily marked both as using endpoint discovery ' . 'and being the endpoint discovery operation. Please ' . 'verify the accuracy of your model files.');
                }
                ...
                // Check/create cache
                if (!isset(self::$cache)) {
                    self::$cache = new \BaghelSoft\S3Uploads\Aws\LruArrayCache($config->getCacheLimit());
                }
                if (empty($endpointList = self::$cache->get($cacheKey))) {
                    $endpointList = new \BaghelSoft\S3Uploads\Aws\EndpointDiscovery\EndpointList([]);
                }
    ...
    private function discoverEndpoint($cacheKey, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (isset($result['Endpoints'])) {
            ...
            $endpointList = new \BaghelSoft\S3Uploads\Aws\EndpointDiscovery\EndpointList($endpointData);
            self::$cache->set($cacheKey, $endpointList);
            return $endpointList->getEndpoint();
        }
        throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException('The endpoint discovery operation ' . 'yielded a response that did not contain properly formatted ' . 'endpoint data.');
    }
    private function getCacheKey(\BaghelSoft\S3Uploads\Aws\Credentials\CredentialsInterface $creds, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
    ...
    private function getDiscoveryCommand(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, array $identifiers)
    {
        ...
        if (!isset($endpointOperation)) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException('This command is set to use ' . 'endpoint discovery, but no endpoint discovery operation was ' . 'found. Please verify the accuracy of your model files.');
        }
        $params = [];
        if (!empty($identifiers)) {
        ...
        $command->getHandlerList()->appendBuild(\BaghelSoft\S3Uploads\Aws\Middleware::mapRequest(function (\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $r) {
    ...
    private function handleDiscoveryException($isRequired, $originalUri, \Exception $e, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($isRequired) {
            ...
            throw new \BaghelSoft\S3Uploads\Aws\Exception\AwsException($message, $cmd, ['code' => 'EndpointDiscoveryException', 'message' => $message], $e);
        }
    ...
    private function modifyRequest(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, $endpoint)
    {
    ...
    private function parseEndpoint($endpoint)
    {
        ...
        throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException("The supplied endpoint '" . "{$endpoint}' is invalid.");
    }
    private function useOriginalUri(\BaghelSoft\S3Uploads\Psr\Http\Message\UriInterface $uri, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        $nextHandler = $this->nextHandler;
