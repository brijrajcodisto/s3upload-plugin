...
use BaghelSoft\S3Uploads\Aws\Api\Validator;
use BaghelSoft\S3Uploads\Aws\Api\ApiProvider;
use BaghelSoft\S3Uploads\Aws\Api\Service;
use BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration;
use BaghelSoft\S3Uploads\Aws\Credentials\Credentials;
...
use BaghelSoft\S3Uploads\Aws\EndpointDiscovery\ConfigurationInterface;
use BaghelSoft\S3Uploads\Aws\EndpointDiscovery\ConfigurationProvider;
use BaghelSoft\S3Uploads\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use BaghelSoft\S3Uploads\Aws\Exception\InvalidRegionException;
use BaghelSoft\S3Uploads\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use BaghelSoft\S3Uploads\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use BaghelSoft\S3Uploads\Aws\Signature\SignatureProvider;
use BaghelSoft\S3Uploads\Aws\Endpoint\EndpointProvider;
use BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \BaghelSoft\S3Uploads\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \BaghelSoft\S3Uploads\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\BaghelSoft\S3Uploads\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\BaghelSoft\S3Uploads\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof CredentialsInterface) {
            $args['credentials'] = \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::fromCredentials(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === false) {
            $args['credentials'] = \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::fromCredentials(new \BaghelSoft\S3Uploads\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof CacheInterface) {
            $args['credentials'] = \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
            throw new \InvalidArgumentException('Credentials must be an instance of ' . 'BaghelSoft\\S3Uploads\\Aws\\Credentials\\CredentialsInterface, an associative ' . 'array that contains "key", "secret", and an optional "token" ' . 'key-value pairs, a credentials provider function, or false.');
        }
    }
    public static function _default_credential_provider(array $args)
    {
        return \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if ($value === false) {
            $value = new \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\Configuration(false, \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \BaghelSoft\S3Uploads\Aws\Api\Service(\BaghelSoft\S3Uploads\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \BaghelSoft\S3Uploads\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \BaghelSoft\S3Uploads\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \BaghelSoft\S3Uploads\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \BaghelSoft\S3Uploads\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \BaghelSoft\S3Uploads\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        $list->prependBuild(\BaghelSoft\S3Uploads\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if ($value !== false) {
            $list->interpose(new \BaghelSoft\S3Uploads\Aws\TraceMiddleware($value === true ? [] : $value, $args['api']));
        }
    }
    public static function _apply_stats($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\BaghelSoft\S3Uploads\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \BaghelSoft\S3Uploads\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if ($value === false) {
            return;
        }
        $validator = $value === true ? new \BaghelSoft\S3Uploads\Aws\Api\Validator() : new \BaghelSoft\S3Uploads\Aws\Api\Validator($value);
        $list->appendValidate(\BaghelSoft\S3Uploads\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {          error_log('inside _default_handler ====>>>>>>>>>>');

        return new \BaghelSoft\S3Uploads\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
     error_log('inside _apply_http_handler ====>>>>>>>>>>');
        $args['handler'] = new \BaghelSoft\S3Uploads\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if (!is_array($value)) {
            $value = [$value];
        }
        ...
        array_unshift($value, 'aws-sdk-php/' . \BaghelSoft\S3Uploads\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', implode(' ', array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        $enabled = false;
        $generator = null;
        if (is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\BaghelSoft\S3Uploads\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        $options = self::getEndpointProviderOptions($args);
        return \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \BaghelSoft\S3Uploads\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \BaghelSoft\S3Uploads\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : call_user_func(\BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \BaghelSoft\S3Uploads\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
