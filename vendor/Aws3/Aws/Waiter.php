 ...
 */
class Waiter implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
        ...
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                if ($state === 'success') {
                    ...
                    (yield new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
