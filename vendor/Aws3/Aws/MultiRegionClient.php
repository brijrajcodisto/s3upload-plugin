...
use BaghelSoft\S3Uploads\Aws\Endpoint\PartitionInterface;
class MultiRegionClient implements \BaghelSoft\S3Uploads\Aws\AwsClientInterface
{
    use AwsClientTrait;
    ...
    {
    error_log('inside multi region getArguments() ====>>>>>>>>>>');
        $args = array_intersect_key(\BaghelSoft\S3Uploads\Aws\ClientResolver::getDefaultArguments(), ['service' => true, 'region' => true]);
        ...
        return $args + ['client_factory' => ['type' => 'config', 'valid' => ['callable'], 'doc' => 'A callable that takes an array of client' . ' configuration arguments and returns a regionalized' . ' client.', 'required' => true, 'internal' => true, 'default' => function (array $args) {
            ...
            $klass = "BaghelSoft\\S3Uploads\\Aws\\{$namespace}\\{$namespace}Client";
        ...
        }], 'partition' => ['type' => 'config', 'valid' => ['string', \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionInterface::class], 'doc' => 'AWS partition to connect to. Valid partitions' . ' include "aws," "aws-cn," and "aws-us-gov." Used to' . ' restrict the scope of the mapRegions method.', 'default' => function (array $args) {
            $region = isset($args['region']) ? $args['region'] : '';
            return \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, $args['service']);
        }, 'fn' => function ($value, array &$args) {
            if (is_string($value)) {
                $value = \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartitionByName($value);
            }
    ...
    public function __construct(array $args = [])
    {
        ...
        $this->handlerList = new \BaghelSoft\S3Uploads\Aws\HandlerList(function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command) {
        ...
        $resolver = new \BaghelSoft\S3Uploads\Aws\ClientResolver($argDefinitions);
