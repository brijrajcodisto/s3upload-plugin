...
namespace BaghelSoft\S3Uploads\Aws\Retry;
...
use BaghelSoft\S3Uploads\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends \BaghelSoft\S3Uploads\Aws\AbstractConfigurationProvider implements \BaghelSoft\S3Uploads\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \BaghelSoft\S3Uploads\Aws\Retry\ConfigurationInterface::class;
    protected static $exceptionClass = \BaghelSoft\S3Uploads\Aws\Retry\Exception\ConfigurationException::class;
    /**
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Retry\Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Retry\Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for(new \BaghelSoft\S3Uploads\Aws\Retry\Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if (is_int($config)) {
            return new \BaghelSoft\S3Uploads\Aws\Retry\Configuration('legacy', $config + 1);
        }
        if (is_array($config) && isset($config['mode'])) {
            $maxAttempts = isset($config['max_attempts']) ? $config['max_attempts'] : self::DEFAULT_MAX_ATTEMPTS;
            return new \BaghelSoft\S3Uploads\Aws\Retry\Configuration($config['mode'], $maxAttempts);
        }
