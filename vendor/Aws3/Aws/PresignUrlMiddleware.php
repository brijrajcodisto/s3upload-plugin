...
use BaghelSoft\S3Uploads\Aws\Signature\SignatureV4;
...
class PresignUrlMiddleware
{
    ...
    public function __construct(array $options, callable $endpointProvider, \BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, callable $nextHandler)
    {
    ...
    public static function wrap(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, callable $endpointProvider, array $options = [])
    {
        return function (callable $handler) use($endpointProvider, $client, $options) {
            $f = new \BaghelSoft\S3Uploads\Aws\PresignUrlMiddleware($options, $endpointProvider, $client, $handler);
            return $f;
        };
    }
    public function __invoke(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function createPresignedUrl(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd)
    {
        $cmdName = $cmd->getName();
        $newCmd = $client->getCommand($cmdName, $cmd->toArray());
        // Avoid s3recursion by flagging the new command.
        $newCmd->{'__skip' . $cmdName} = true;
        // Serialize a request for the operation.
        $request = \BaghelSoft\S3Uploads\Aws\serialize($newCmd);
        // Create the new endpoint for the target endpoint.
        $endpoint = \BaghelSoft\S3Uploads\Aws\Endpoint\EndpointProvider::resolve($this->endpointProvider, ['region' => $cmd['SourceRegion'], 'service' => $this->serviceName])['endpoint'];
        // Set the request to hit the target endpoint.
        $uri = $request->getUri()->withHost((new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Uri($endpoint))->getHost());
        $request = $request->withUri($uri);
        // Create a presigned URL for our generated request.
        $signer = new \BaghelSoft\S3Uploads\Aws\Signature\SignatureV4($this->serviceName, $cmd['SourceRegion']);
        ...
        return (string) $signer->presign(\BaghelSoft\S3Uploads\Aws\Signature\SignatureV4::convertPostToGet($request, $paramsToAdd ?: ""), $client->getCredentials()->wait(), '+1 hour')->getUri();
    }
}
