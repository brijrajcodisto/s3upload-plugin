...
use BaghelSoft\S3Uploads\Aws\ClientSideMonitoring\ConfigurationProvider;
 ...
 */
class AwsClient implements \BaghelSoft\S3Uploads\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $aliases;
    ...
    {  error_log('inside getArguments() ====>>>>>>>>>>');
        return \BaghelSoft\S3Uploads\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \BaghelSoft\S3Uploads\Aws\HandlerList();
        $resolver = new \BaghelSoft\S3Uploads\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \BaghelSoft\S3Uploads\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'BaghelSoft\\S3Uploads\\Aws\\Exception\\AwsException'];
        }
        $service = substr($klass, strrpos($klass, '\\') + 1, -6);
        return [strtolower($service), "BaghelSoft\\S3Uploads\\Aws\\{$service}\\Exception\\{$service}Exception"];
    }
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            $list = $this->getHandlerList();
            $list->appendBuild(\BaghelSoft\S3Uploads\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addEndpointDiscoveryMiddleware($config, $args)
    {
        ...
        if (!isset($args['endpoint'])) {
            $list->appendBuild(\BaghelSoft\S3Uploads\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware::wrap($this, $args, $config['endpoint_discovery']), 'EndpointDiscoveryMiddleware');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\BaghelSoft\S3Uploads\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \BaghelSoft\S3Uploads\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\BaghelSoft\S3Uploads\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\BaghelSoft\S3Uploads\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \BaghelSoft\S3Uploads\Aws\load_compiled_json($file);
    ...
    {
        $streamRequestPayloadMiddleware = \BaghelSoft\S3Uploads\Aws\StreamRequestPayloadMiddleware::wrap($this->api);
    ...
    {
        $aliases = \BaghelSoft\S3Uploads\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
