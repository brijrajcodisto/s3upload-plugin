...
abstract class AbstractUploader extends \BaghelSoft\S3Uploads\Aws\Multipart\AbstractUploadManager
{
     ...
     * @param array  $config
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $source, array $config = [])
    {
     ...
     */
    protected function limitPartStream(\BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface $stream)
    {
        // Limit what is read from the stream to the part size.
        return new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\LimitStream($stream, $this->state->getPartSize(), $this->source->tell());
    }
        ...
        for ($partNumber = 1; $this->isEof($seekable); $partNumber++) {
            ...
            if (!$this->state->hasPartBeenUploaded($partNumber)) {
                ...
                if (isset($numberOfParts) && $partNumber > $numberOfParts) {
                    throw new $this->config['exception_class']($this->state, new \BaghelSoft\S3Uploads\Aws\Exception\AwsException("Maximum part number for this job exceeded, file has likely been corrupted." . "  Please restart this upload.", $command));
                }
                (yield $command);
    ...
    private function determineSource($source)
    {
        ...
        if (is_string($source)) {
            $source = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\try_fopen($source, 'r');
        }
        // Create a source stream.
        $stream = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\stream_for($source);
