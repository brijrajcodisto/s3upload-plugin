...
namespace BaghelSoft\S3Uploads\Aws\Multipart;

use BaghelSoft\S3Uploads\Aws\AwsClientInterface as Client;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'BaghelSoft\\S3Uploads\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\BaghelSoft\S3Uploads\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \BaghelSoft\S3Uploads\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\BaghelSoft\S3Uploads\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        $state = new \BaghelSoft\S3Uploads\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\BaghelSoft\S3Uploads\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \BaghelSoft\S3Uploads\Aws\Result();
                });
            };
        };
    }
