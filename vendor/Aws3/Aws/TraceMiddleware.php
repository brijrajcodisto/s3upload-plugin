...
class TraceMiddleware
{
     ...
     *   "scrub_auth" is set to true.
     */
    public function __construct(array $config = [], \BaghelSoft\S3Uploads\Aws\Api\Service $service = null)
    {
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd)
    {
        return ['instance' => spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $this->getRedactedArray($cmd)];
    }
    private function requestArray(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\BaghelSoft\S3Uploads\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    private function streamStr(\BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
     ...
     * @return array
     */
    private function getRedactedArray(\BaghelSoft\S3Uploads\Aws\CommandInterface $cmd)
    {
