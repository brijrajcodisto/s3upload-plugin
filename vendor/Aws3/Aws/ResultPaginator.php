...
class ResultPaginator implements \Iterator
{
     ...
     * @param array              $config
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $operation, array $args, array $config)
    {
        $this->client = $client;
    ...
    {
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () use($handleResult) {
            $nextToken = null;
            do {
                ...
                if ($retVal !== null) {
                    (yield \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($retVal));
                }
            } while ($nextToken);
        });
    }
    ...
    public function search($expression)
    {
        ...
        return flatmap($this, function (\BaghelSoft\S3Uploads\Aws\Result $result) use($expression) {
    ...
    private function determineNextToken(\BaghelSoft\S3Uploads\Aws\Result $result)
    {
