...
use BaghelSoft\S3Uploads\Aws\Retry\ConfigurationInterface;
use BaghelSoft\S3Uploads\Aws\Retry\QuotaManager;
use BaghelSoft\S3Uploads\Aws\Retry\RateLimiter;
use BaghelSoft\S3Uploads\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\BaghelSoft\S3Uploads\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (extension_loaded('curl')) {
            $retryCurlErrors[CURLE_RECV_ERROR] = true;
        }
        return function ($attempts, \BaghelSoft\S3Uploads\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
    ...
    public function __construct(\BaghelSoft\S3Uploads\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \BaghelSoft\S3Uploads\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \BaghelSoft\S3Uploads\Aws\Retry\RateLimiter();
        }
    }
        ...
        $callback = function ($value) use($handler, $cmd, $req, $decider, $delayer, &$attempts, &$requestStats, &$monitoringEvents, &$callback) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($attempts, $cmd, $value)) {
                    return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
