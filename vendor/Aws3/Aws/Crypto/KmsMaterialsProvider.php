 ...
 * @deprecated
 */
class KmsMaterialsProvider extends \BaghelSoft\S3Uploads\Aws\Crypto\MaterialsProvider implements \BaghelSoft\S3Uploads\Aws\Crypto\MaterialsProviderInterface
{
    ...
    public function fromDecryptionEnvelope(\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect the materials description.');
        }
        $materialsDescription = json_decode($envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], true);
