...
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7\AppendStream;
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Stream;
trait EncryptionTrait
{
     ...
     * @internal
     */
    public function encrypt(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \BaghelSoft\S3Uploads\Aws\Crypto\MaterialsProvider $provider, \BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\BaghelSoft\S3Uploads\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::IV_HEADER] = base64_encode($cipherOptions['Iv']);
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = strlen($plaintext);
        $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \BaghelSoft\S3Uploads\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                ...
                $appendStream = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\stream_for($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \BaghelSoft\S3Uploads\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
