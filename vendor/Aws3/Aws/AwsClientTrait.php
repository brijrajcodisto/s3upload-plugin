...
trait AwsClientTrait
{
    ...
    {
    //echo 'Aws get paginator====>>>>>>';
        $config = $this->getApi()->getPaginatorConfig($name);
        return new \BaghelSoft\S3Uploads\Aws\ResultPaginator($this, $name, $args, $config);
    }
    ...
    public function getWaiter($name, array $args = [])
    {
        ...
        return new \BaghelSoft\S3Uploads\Aws\Waiter($this, $name, $args, $config);
    }
    public function execute(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        return $this->executeAsync($command)->wait();
    }
    public function executeAsync(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
