...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, \BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \BaghelSoft\S3Uploads\Aws\S3\S3Client::encodeKey(rawurldecode($path));
        }
