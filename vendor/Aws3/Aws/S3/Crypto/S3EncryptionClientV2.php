...
use BaghelSoft\S3Uploads\Aws\Crypto\DecryptionTraitV2;
...
use BaghelSoft\S3Uploads\Aws\Crypto\AbstractCryptoClientV2;
use BaghelSoft\S3Uploads\Aws\Crypto\EncryptionTraitV2;
use BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope;
use BaghelSoft\S3Uploads\Aws\Crypto\MaterialsProvider;
use BaghelSoft\S3Uploads\Aws\Crypto\Cipher\CipherBuilderTrait;
 ...
 * </code>
 */
class S3EncryptionClientV2 extends \BaghelSoft\S3Uploads\Aws\Crypto\AbstractCryptoClientV2
{
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
    error_log('inside en clientv2 ====>>>>>>>>>>');
    ...
    public function putObjectAsync(array $args)
    {
        ...
        $envelope = new \BaghelSoft\S3Uploads\Aws\Crypto\MetadataEnvelope();
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($this->encrypt(\BaghelSoft\S3Uploads\GuzzleHttp\Psr7\stream_for($args['Body']), $args, $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \BaghelSoft\S3Uploads\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \BaghelSoft\S3Uploads\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
    ...
    public function getObjectAsync(array $args)
    {
        ...
        if (!isset($args['@SecurityProfile']) || !in_array($args['@SecurityProfile'], self::$supportedSecurityProfiles)) {
            throw new \BaghelSoft\S3Uploads\Aws\Exception\CryptoException("@SecurityProfile is required and must be" . " set to 'V2' or 'V2_AND_LEGACY'");
        }
