...
use BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface;
use Iterator;
/**
 ...
 */
class Transfer implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    private $client;
    private $promise;
    private $source;
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface $client, $source, $dest, array $options = [])
    {
        $this->client = $client;
        ...
        $this->concurrency = isset($options['concurrency']) ? $options['concurrency'] : \BaghelSoft\S3Uploads\Aws\S3\MultipartUploader::DEFAULT_CONCURRENCY;
        $this->mupThreshold = isset($options['mup_threshold']) ? $options['mup_threshold'] : 16777216;
        if ($this->mupThreshold < \BaghelSoft\S3Uploads\Aws\S3\MultipartUploader::PART_MIN_SIZE) {
    ...
    private function createDownloadPromise()
    {
        ...
        foreach ($this->getDownloadsIterator() as $object) {
            ...
            if (strpos($this->resolveUri($resolveSink), $this->destination['path']) !== 0) {
                throw new \BaghelSoft\S3Uploads\Aws\Exception\AwsException('Cannot download key ' . $objectKey . ', its relative path resolves outside the' . ' parent directory', $command);
            }
        ...
        return (new \BaghelSoft\S3Uploads\Aws\CommandPool($this->client, $commands, ['concurrency' => $this->concurrency, 'before' => $this->before, 'rejected' => function ($reason, $idx, \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromiseInterface $p) {
            $p->reject($reason);
        }]))->promise();
    }
    private function createUploadPromise()
    {
        ...
        $files = \BaghelSoft\S3Uploads\Aws\map($this->getUploadsIterator(), function ($file) {
        ...
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\each_limit_all($files, $this->concurrency);
    }
    /** @return Iterator */
    private function getUploadsIterator()
    {
        if (is_string($this->source)) {
            return \BaghelSoft\S3Uploads\Aws\filter(\BaghelSoft\S3Uploads\Aws\recursive_dir_iterator($this->sourceMetadata['path']), function ($file) {
                return !is_dir($file);
            });
        }
        return $this->source;
    ...
    private function getDownloadsIterator()
    {
        if (is_string($this->source)) {
            ...
            $files = \BaghelSoft\S3Uploads\Aws\map($files, function ($key) use($listArgs) {
                return "s3://{$listArgs['Bucket']}/{$key}";
            });
            return \BaghelSoft\S3Uploads\Aws\filter($files, function ($key) {
    ...
    private function uploadMultipart($filename)
    {
        ...
        return (new \BaghelSoft\S3Uploads\Aws\S3\MultipartUploader($this->client, $filename, ['bucket' => $args['Bucket'], 'key' => $args['Key'], 'before_initiate' => $this->before, 'before_upload' => $this->before, 'before_complete' => $this->before, 'concurrency' => $this->concurrency]))->promise();
    }
    ...
    private function addDebugToBefore($debug)
    {
        ...
        $s3Args = $this->s3Args;
        $this->before = static function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command) use($before, $debug, $sourcePath, $s3Args) {
