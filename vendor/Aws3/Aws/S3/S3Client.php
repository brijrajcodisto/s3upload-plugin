...
use BaghelSoft\S3Uploads\Aws\Api\DocModel;
use BaghelSoft\S3Uploads\Aws\Api\Service;
use BaghelSoft\S3Uploads\Aws\AwsClient;
use BaghelSoft\S3Uploads\Aws\CacheInterface;
use BaghelSoft\S3Uploads\Aws\ClientResolver;
use BaghelSoft\S3Uploads\Aws\Command;
use BaghelSoft\S3Uploads\Aws\Exception\AwsException;
use BaghelSoft\S3Uploads\Aws\HandlerList;
use BaghelSoft\S3Uploads\Aws\InputValidationMiddleware;
use BaghelSoft\S3Uploads\Aws\Middleware;
use BaghelSoft\S3Uploads\Aws\Retry\QuotaManager;
use BaghelSoft\S3Uploads\Aws\RetryMiddleware;
use BaghelSoft\S3Uploads\Aws\ResultInterface;
use BaghelSoft\S3Uploads\Aws\CommandInterface;
use BaghelSoft\S3Uploads\Aws\RetryMiddlewareV2;
use BaghelSoft\S3Uploads\Aws\S3\UseArnRegion\Configuration;
use BaghelSoft\S3Uploads\Aws\S3\UseArnRegion\ConfigurationInterface;
use BaghelSoft\S3Uploads\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use BaghelSoft\S3Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use BaghelSoft\S3Uploads\GuzzleHttp\Exception\RequestException;
use BaghelSoft\S3Uploads\GuzzleHttp\Promise\Promise;
use BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromiseInterface;
 ...
 */
class S3Client extends \BaghelSoft\S3Uploads\Aws\AwsClient implements \BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var array */
    ...
    {            error_log('inside S3Client construct ====>>>>>>>>>>');

        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \BaghelSoft\S3Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\BaghelSoft\S3Uploads\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\BaghelSoft\S3Uploads\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\BaghelSoft\S3Uploads\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\BaghelSoft\S3Uploads\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\BaghelSoft\S3Uploads\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\BaghelSoft\S3Uploads\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendValidate(\BaghelSoft\S3Uploads\Aws\InputValidationMiddleware::wrap($this->getApi(), self::$mandatoryAttributes), 'input_validation_middleware');
        $stack->appendSign(\BaghelSoft\S3Uploads\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\BaghelSoft\S3Uploads\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\BaghelSoft\S3Uploads\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if ($value instanceof CacheInterface) {
            $value = \BaghelSoft\S3Uploads\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        ...
        if ($value instanceof ConfigurationInterface) {
            ...
            $args['use_arn_region'] = new \BaghelSoft\S3Uploads\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\BaghelSoft\S3Uploads\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \BaghelSoft\S3Uploads\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\BaghelSoft\S3Uploads\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\BaghelSoft\S3Uploads\Aws\Command $command, $request = null) use($handler) {
    ...
    {
        return static function (callable $handler) {
            return function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\BaghelSoft\S3Uploads\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\BaghelSoft\S3Uploads\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \BaghelSoft\S3Uploads\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \BaghelSoft\S3Uploads\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                ...
                $delay = [\BaghelSoft\S3Uploads\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\BaghelSoft\S3Uploads\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \BaghelSoft\S3Uploads\Aws\RetryMiddlewareV2::createDefaultDecider(new \BaghelSoft\S3Uploads\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\BaghelSoft\S3Uploads\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \BaghelSoft\S3Uploads\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \BaghelSoft\S3Uploads\Aws\HandlerList $list)
    {
        \BaghelSoft\S3Uploads\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \BaghelSoft\S3Uploads\Aws\S3\GetBucketLocationParser(new \BaghelSoft\S3Uploads\Aws\S3\AmbiguousSuccessParser(new \BaghelSoft\S3Uploads\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \BaghelSoft\S3Uploads\Aws\Api\Service($api, \BaghelSoft\S3Uploads\Aws\Api\ApiProvider::defaultProvider()), new \BaghelSoft\S3Uploads\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
