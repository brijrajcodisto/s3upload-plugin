...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\BaghelSoft\S3Uploads\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \BaghelSoft\S3Uploads\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => false, 'region' => $arn->getRegion()];
            }
        }
        $url = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\uri_for($uri);
    ...
    private function parseStreamWrapper(\BaghelSoft\S3Uploads\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\BaghelSoft\S3Uploads\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\BaghelSoft\S3Uploads\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\BaghelSoft\S3Uploads\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = false;
