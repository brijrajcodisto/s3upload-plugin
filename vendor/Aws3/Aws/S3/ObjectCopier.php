...
use BaghelSoft\S3Uploads\Aws\Arn\S3\AccessPointArn;
 ...
 * when appropriate.
 */
class ObjectCopier implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \BaghelSoft\S3Uploads\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \BaghelSoft\S3Uploads\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    {
        if (\BaghelSoft\S3Uploads\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \BaghelSoft\S3Uploads\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
            } catch (\Exception $e) {
