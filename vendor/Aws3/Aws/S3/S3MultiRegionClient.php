...
use BaghelSoft\S3Uploads\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \BaghelSoft\S3Uploads\Aws\MultiRegionClient implements \BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\BaghelSoft\S3Uploads\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \BaghelSoft\S3Uploads\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\BaghelSoft\S3Uploads\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () use($handler, $command, $cacheKey) {
                    try {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($cached);
        }
