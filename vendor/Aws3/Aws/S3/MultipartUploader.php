...
use BaghelSoft\S3Uploads\Aws\HashingStream;
use BaghelSoft\S3Uploads\Aws\Multipart\AbstractUploader;
use BaghelSoft\S3Uploads\Aws\PhpHash;
use BaghelSoft\S3Uploads\Aws\ResultInterface;
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7;
use BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface as Stream;
use BaghelSoft\S3Uploads\Aws\S3\Exception\S3MultipartUploadException;
/**
 * Encapsulates the execution of a multipart upload to S3 or Glacier.
 */
class MultipartUploader extends \BaghelSoft\S3Uploads\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \BaghelSoft\S3Uploads\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
            ...
            $body = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\stream_for();
            \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    protected function extractETag(\BaghelSoft\S3Uploads\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
     ...
     * @return Stream
     */
    private function decorateWithHashes(\BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Decorate source with a hashing stream
        $hash = new \BaghelSoft\S3Uploads\Aws\PhpHash('sha256');
        return new \BaghelSoft\S3Uploads\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
