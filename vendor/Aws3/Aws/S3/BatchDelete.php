...
use BaghelSoft\S3Uploads\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 */
class BatchDelete implements \BaghelSoft\S3Uploads\GuzzleHttp\Promise\PromisorInterface
{
    private $bucket;
     ...
     * @return BatchDelete
     */
    public static function fromListObjects(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, array $listObjectsParams, array $options = [])
    {
        $iter = $client->getPaginator('ListObjects', $listObjectsParams);
        $bucket = $listObjectsParams['Bucket'];
        $fn = function (\BaghelSoft\S3Uploads\Aws\S3\BatchDelete $that) use($iter) {
            return $iter->each(function ($result) use($that) {
                ...
                return $promises ? \BaghelSoft\S3Uploads\GuzzleHttp\Promise\all($promises) : null;
            });
        };
     ...
     * @return BatchDelete
     */
    public static function fromIterator(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $bucket, \Iterator $iter, array $options = [])
    {
        $fn = function (\BaghelSoft\S3Uploads\Aws\S3\BatchDelete $that) use($iter) {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\coroutine(function () use($that, $iter) {
                foreach ($iter as $obj) {
     ...
     */
    private function __construct(\BaghelSoft\S3Uploads\Aws\AwsClientInterface $client, $bucket, callable $promiseFn, array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
    ...
    private function flushQueue()
    {
        ...
        return $this->client->executeAsync($command)->then(function ($result) {
            if (!empty($result['Errors'])) {
                throw new \BaghelSoft\S3Uploads\Aws\S3\Exception\DeleteMultipleObjectsException($result['Deleted'] ?: [], $result['Errors']);
            }
            return $result;
        });
    }
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return \BaghelSoft\S3Uploads\GuzzleHttp\Promise\promise_for($this->flushQueue())->then($cleanup);
