...
class StreamWrapper
{
     ...
     */
    public static function register(\BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface $client, $protocol = 's3', \BaghelSoft\S3Uploads\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \BaghelSoft\S3Uploads\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\mimetype_from_filename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \BaghelSoft\S3Uploads\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\BaghelSoft\S3Uploads\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return true;
    }
    private function openWriteStream()
    {
        $this->body = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Stream(fopen('php://temp', 'r+'));
        return true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \BaghelSoft\S3Uploads\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
