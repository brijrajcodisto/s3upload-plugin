...
use BaghelSoft\S3Uploads\Aws\Multipart\AbstractUploadManager;
use BaghelSoft\S3Uploads\Aws\ResultInterface;
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7;
class MultipartCopy extends \BaghelSoft\S3Uploads\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\BaghelSoft\S3Uploads\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    private function fetchSourceMetadata()
    {
        ...
        if (strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\parse_query($query, false);
