...
use BaghelSoft\S3Uploads\GuzzleHttp\Exception\InvalidArgumentException;
...
class S3EndpointMiddleware
{
    ...
    public function __construct(callable $nextHandler, $region, array $options, $endpointProvider = null)
    {
        ...
        $this->endpointProvider = is_null($endpointProvider) ? \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider() : $endpointProvider;
    ...
    private static function isRequestHostStyleCompatible(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        return \BaghelSoft\S3Uploads\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || strpos($command['Bucket'], '.') === false) && filter_var($request->getUri()->getHost(), FILTER_VALIDATE_IP) === false;
    }
    private function endpointPatternDecider(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \BaghelSoft\S3Uploads\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, $pattern)
    {
        $dnsSuffix = $this->endpointProvider->getPartition($this->region, 's3')->getDnsSuffix();
        return "{$command['Bucket']}.{$pattern}.{$dnsSuffix}";
    }
    private function getBucketlessPath($path, \BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        $pattern = '/^\\/' . preg_quote($command['Bucket'], '/') . '/';
        return preg_replace($pattern, '', $path) ?: '/';
    }
    private function applyEndpoint(\BaghelSoft\S3Uploads\Aws\CommandInterface $command, \BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request)
    {
        $dualStack = isset($command['@use_dual_stack_endpoint']) ? $command['@use_dual_stack_endpoint'] : $this->dualStackByDefault;
        if (\BaghelSoft\S3Uploads\Aws\Arn\ArnParser::isArn($command['Bucket'])) {
            $arn = \BaghelSoft\S3Uploads\Aws\Arn\ArnParser::parse($command['Bucket']);
            $outpost = $arn->getService() == 's3-outposts';
            if ($outpost && $dualStack) {
                throw new \BaghelSoft\S3Uploads\GuzzleHttp\Exception\InvalidArgumentException("Outposts + dualstack is not supported");
            }
        }
        if ($dualStack) {
            throw new \BaghelSoft\S3Uploads\GuzzleHttp\Exception\InvalidArgumentException("Custom Endpoint + Dualstack not supported");
        }
        $host = $this->pathStyleByDefault ? $this->endpoint : $this->getBucketStyleHost($command, $this->endpoint);
        $uri = new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Uri($host);
