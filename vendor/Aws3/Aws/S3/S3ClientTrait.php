...
use BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise;
...
trait S3ClientTrait
{
    ...
    {
        return (new \BaghelSoft\S3Uploads\Aws\S3\ObjectUploader($this, $bucket, $key, $body, $acl, $options))->promise();
    }
    /**
    ...
    public function copyAsync($fromB, $fromK, $destB, $destK, $acl = 'private', array $opts = [])
    {
        ...
        return (new \BaghelSoft\S3Uploads\Aws\S3\ObjectCopier($this, $source, $destination, $acl, $opts))->promise();
    }
    /**
    ...
    {
        \BaghelSoft\S3Uploads\Aws\S3\StreamWrapper::register($this);
    }
    /**
    ...
    {
        if (!$prefix && !$regex) {
            return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException('A prefix or regex is required.'));
        }
        $params = ['Bucket' => $bucket, 'Prefix' => $prefix];
        $iter = $this->getIterator('ListObjects', $params);
        if ($regex) {
            $iter = \BaghelSoft\S3Uploads\Aws\filter($iter, function ($c) use($regex) {
                return preg_match($regex, $c['Key']);
            });
        }
        return \BaghelSoft\S3Uploads\Aws\S3\BatchDelete::fromIterator($this, $bucket, $iter, $options)->promise();
    }
    /**
    ...
    public function uploadDirectoryAsync($directory, $bucket, $keyPrefix = null, array $options = [])
    {
        ...
        return (new \BaghelSoft\S3Uploads\Aws\S3\Transfer($this, $directory, $d, $options))->promise();
    }
    /**
    ...
    public function downloadBucketAsync($directory, $bucket, $keyPrefix = '', array $options = [])
    {
        ...
        return (new \BaghelSoft\S3Uploads\Aws\S3\Transfer($this, $s, $directory, $options))->promise();
    }
    /**
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        return $handler($command)->then(static function (\BaghelSoft\S3Uploads\Aws\ResultInterface $result) {
            return $result['@metadata']['headers']['x-amz-bucket-region'];
        }, function (\BaghelSoft\S3Uploads\Aws\Exception\AwsException $e) {
    ...
    private function determineBucketRegionFromExceptionBody(\BaghelSoft\S3Uploads\Psr\Http\Message\ResponseInterface $response)
    {
        try {
     ...
     */
    private function checkExistenceWithCommand(\BaghelSoft\S3Uploads\Aws\CommandInterface $command)
    {
        try {
     ...
     */
    public abstract function execute(\BaghelSoft\S3Uploads\Aws\CommandInterface $command);
    /**
