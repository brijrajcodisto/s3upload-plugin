...
namespace BaghelSoft\S3Uploads\Aws\Endpoint;

use ArrayAccess;
use BaghelSoft\S3Uploads\Aws\HasDataTrait;
use BaghelSoft\S3Uploads\Aws\Sts\RegionalEndpoints\ConfigurationProvider;
use BaghelSoft\S3Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider as S3ConfigurationProvider;
use InvalidArgumentException as Iae;
/**
 * Default implementation of an AWS partition.
 */
final class Partition implements \ArrayAccess, \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionInterface
{
    use HasDataTrait;
    ...
    {
        return $service === 'sts' && in_array($region, $this->stsLegacyGlobalRegions) && (empty($options['sts_regional_endpoints']) || \BaghelSoft\S3Uploads\Aws\Sts\RegionalEndpoints\ConfigurationProvider::unwrap($options['sts_regional_endpoints'])->getEndpointsType() !== 'regional');
    }
    /**
    ...
    {
        return $service === 's3' && $region === 'us-east-1' && (empty($options['s3_us_east_1_regional_endpoint']) || \BaghelSoft\S3Uploads\Aws\S3\RegionalEndpoint\ConfigurationProvider::unwrap($options['s3_us_east_1_regional_endpoint'])->getEndpointsType() !== 'regional');
    }
