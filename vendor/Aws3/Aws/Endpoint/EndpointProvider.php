...
class EndpointProvider
{
    ...
    public static function resolve(callable $provider, array $args = [])
    {
        ...
        throw new \BaghelSoft\S3Uploads\Aws\Exception\UnresolvedEndpointException('Unable to resolve an endpoint using the provider arguments: ' . json_encode($args) . '. Note: you can provide an "endpoint" ' . 'option to a client constructor to bypass invoking an endpoint ' . 'provider.');
    }
    /**
    ...
    {
        return \BaghelSoft\S3Uploads\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
    }
    /**
    ...
    {
        return new \BaghelSoft\S3Uploads\Aws\Endpoint\PatternEndpointProvider($patterns);
    }
}
