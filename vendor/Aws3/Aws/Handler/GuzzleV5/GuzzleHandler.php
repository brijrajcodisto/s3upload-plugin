...
use BaghelSoft\S3Uploads\GuzzleHttp\Event\EndEvent;
...
use BaghelSoft\S3Uploads\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use BaghelSoft\S3Uploads\GuzzleHttp\Promise;
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Response as Psr7Response;
use BaghelSoft\S3Uploads\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\BaghelSoft\S3Uploads\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof Psr7StreamInterface) {
                        $exception->getResponse()->setBody(\BaghelSoft\S3Uploads\GuzzleHttp\Stream\Stream::factory(file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \BaghelSoft\S3Uploads\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\BaghelSoft\S3Uploads\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof Psr7StreamInterface ? new \BaghelSoft\S3Uploads\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\BaghelSoft\S3Uploads\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \BaghelSoft\S3Uploads\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \BaghelSoft\S3Uploads\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\BaghelSoft\S3Uploads\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \BaghelSoft\S3Uploads\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \BaghelSoft\S3Uploads\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
