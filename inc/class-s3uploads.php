...
use BaghelSoft\S3Uploads\Aws\S3\S3Client;
use BaghelSoft\S3Uploads\Aws\Multipart\UploadState;
...
class S3Uploads {
	...
	public function __construct() {
		/**
		 * Filters the capability that is checked for access to s3Uploads settings page.
		 *
		 ...
		 * @since  1.0
		 * @hook   s3uploads_settings_capability
		 *
		 */
		$this->capability = apply_filters( 's3uploads_settings_capability', ( is_multisite() ? 'manage_network_options' : 'manage_options' ) );
	...
	public function get_multipart_upload_state( $key, $upload_id ) {
         error_log('inside get_multipart_upload_state ====>>>>>>>>>>');
	...
	/**
	 * Setup the hooks, urls filtering etc for s3Uploads
	 */
	public function setup() {
global $wpdb;

		$this->admin = S3Uploads_Admin::get_instance();
		...
		if ( $api_data && isset( $api_data->site ) && isset( $api_data->site->upload_key ) ) {
			...
			add_filter( 's3uploads_s3_client_params', function ( $params ) use ( $api_data ) {
		...
		if ( ( ! defined( 'S3UPLOADS_DISABLE_REPLACE_UPLOAD_URL' ) || ! S3UPLOADS_DISABLE_REPLACE_UPLOAD_URL ) && $api_data->site->cdn_enabled ) {
			...
			new S3Uploads_Rewriter( $original_root_dirs['baseurl'], $replacements, $cdn_url );
		}
	}
	...
	public function register_stream_wrapper() {
		/**
		 * S3UPLOADS_USE_LOCAL define. If true will use the local stream wrapper to write files to local directory instead of cloud.
		 *
		 * @constant {boolean} S3UPLOADS_USE_LOCAL
		 * @default false
		 */
		if ( defined( 'S3UPLOADS_USE_LOCAL' ) && S3UPLOADS_USE_LOCAL ) {
			stream_wrapper_register( 'iu', 'S3Uploads_Local_Stream_Wrapper', STREAM_IS_URL );
		} else {
 	         error_log('inside register_stream_wrapper ====>>>>>>>>>>');

			S3Uploads_Stream_Wrapper::register( $this->s3() );
			/**
			 * S3UPLOADS_OBJECT_ACL define. If set will override the object ACL for new objects stored in the cloud.
			 *
			 * @constant {string} S3UPLOADS_OBJECT_ACL
			 * @default `public-read`
			 */
			$objectAcl = defined( 'S3UPLOADS_OBJECT_ACL' ) ? S3UPLOADS_OBJECT_ACL : 'public-read';
	...
	public function stream_wrapper_debug() {
		if ( $this->stream_api_call_count['total'] ) {
			error_log( sprintf( "[S3UPLOADS Stream Debug] Stream wrapper API calls in %ss: %s", timer_stop(), json_encode( $this->stream_api_call_count, JSON_PRETTY_PRINT ) ) );
		}
		if ( count( $this->stream_plugin_api_call_count ) ) {
			error_log( sprintf( "[S3UPLOADS Stream Debug] Stream wrapper API calls by plugin: %s", json_encode( $this->stream_plugin_api_call_count, JSON_PRETTY_PRINT ) ) );
		}
	}

	/**
	 * @return BaghelSoft\S3Uploads\Aws\S3\S3Client
	 */
	public function s3() {
		 ...
		 * @since  1.0
		 * @hook   s3uploads_s3_client_params
		 *
		 */
		$params   = apply_filters( 's3uploads_s3_client_params', $params );
 	error_log('inside S3Client call ====>>>>>>>>>>');
	...
	public function setup_notice() {
		...
		if ( get_current_screen()->id == 'media_page_s3uploads' || get_current_screen()->id == 'settings_page_s3uploads-network' ) {
			return;
		}
		?>
		<div class="notice notice-info" style="white-space: nowrap;padding: 10px 15px 10px 10px;">
			<span style="display: inline-block;vertical-align: middle;white-space: normal;width: 80%;font-size: 15px;">
				<strong><?php esc_html_e( 's3Uploads is almost ready!', 's3uploads' ); ?></strong>
				<?php
				if ( $this->api->has_token() ) {
					esc_html_e( 'Finish syncing your images, audio, video, and documents to the cloud to enable.', 's3uploads' );
				} else {
					esc_html_e( 'Create or connect your account to move your images, audio, video, and documents to the cloud - with a click!', 's3uploads' );
				}
				?>
			</span>
			<span style="display: inline-block;vertical-align: middle;width: 20%;text-align: right;">
				<a class="button button-primary" href="<?php echo esc_url( $this->admin->settings_url() ); ?>" style="font-size: 15px;"><?php echo $this->api->has_token() ? esc_html__( 'Finish Sync', 's3uploads' ) : esc_html__( 'Connect', 's3uploads' ); ?></a>
			</span>
		</div>
		<?php
	}
	...
	/**
	 * Tear down the hooks, url filtering etc for s3Uploads
	 */
	...
	public function get_sync_stats() {
		...
		$total     = $wpdb->get_row( "SELECT count(*) AS files, SUM(`size`) as size, SUM(`transferred`) as transferred FROM `{$wpdb->base_prefix}s3uploads_files` WHERE 1" );
		$local     = $wpdb->get_row( "SELECT count(*) AS files, SUM(`size`) as size, SUM(`transferred`) as transferred FROM `{$wpdb->base_prefix}s3uploads_files` WHERE deleted = 0" );
		$synced    = $wpdb->get_row( "SELECT count(*) AS files, SUM(`size`) as size, SUM(`transferred`) as transferred FROM `{$wpdb->base_prefix}s3uploads_files` WHERE synced = 1" );
		$deletable = $wpdb->get_row( "SELECT count(*) AS files, SUM(`size`) as size, SUM(`transferred`) as transferred FROM `{$wpdb->base_prefix}s3uploads_files` WHERE synced = 1 AND deleted = 0" );
		$deleted   = $wpdb->get_row( "SELECT count(*) AS files, SUM(`size`) as size, SUM(`transferred`) as transferred FROM `{$wpdb->base_prefix}s3uploads_files` WHERE synced = 1 AND deleted = 1" );
	...
	public function get_filetypes( $is_chart = false, $cloud_types = false ) {
		...
		if ( false !== $cloud_types ) {
			if ( empty( $cloud_types ) ) { //estimate if sync was fresh
				$types = $wpdb->get_results( "SELECT type, count(*) AS files, SUM(`size`) as size FROM `{$wpdb->base_prefix}s3uploads_files` WHERE synced = 1 GROUP BY type ORDER BY size DESC" );
			} else {
				$types = $cloud_types;
			}
		} else {
			$types = $wpdb->get_results( "SELECT type, count(*) AS files, SUM(`size`) as size FROM `{$wpdb->base_prefix}s3uploads_files` WHERE deleted = 0 GROUP BY type ORDER BY size DESC" );
		}
		...
		if ( $is_chart ) {
			foreach ( $data as $item ) {
				...
				$chart['labels'][]                         = $item->label . ": " . sprintf( _n( '%s file totalling %s', '%s files totalling %s', $item->files, 's3uploads' ), number_format_i18n( $item->files ), size_format( $item->size, 1 ) );
			}

			$total_size     = array_sum( wp_list_pluck( $data, 'size' ) );
			$total_files    = array_sum( wp_list_pluck( $data, 'files' ) );
			$chart['total'] = sprintf( _n( '%s / %s File', '%s / %s Files', $total_files, 's3uploads' ), size_format( $total_size, 2 ), number_format_i18n( $total_files ) );
	...
	public function get_file_type_format( $type, $key ) {
		$labels = [
			'image'    => [ 'color' => '#26A9E0', 'label' => esc_html__( 'Images', 's3uploads' ) ],
			'audio'    => [ 'color' => '#00A167', 'label' => esc_html__( 'Audio', 's3uploads' ) ],
			'video'    => [ 'color' => '#C035E2', 'label' => esc_html__( 'Video', 's3uploads' ) ],
			'document' => [ 'color' => '#EE7C1E', 'label' => esc_html__( 'Documents', 's3uploads' ) ],
			'archive'  => [ 'color' => '#EC008C', 'label' => esc_html__( 'Archives', 's3uploads' ) ],
			'code'     => [ 'color' => '#EFED27', 'label' => esc_html__( 'Code', 's3uploads' ) ],
			'other'    => [ 'color' => '#F1F1F1', 'label' => esc_html__( 'Other', 's3uploads' ) ],
		];
	...
	public function get_files_for_unique_filename_file_list( $files, $dir, $filename ) {
		...
		error_log(sprintf('inside get_files_for_unique_filename_file_list ====>>>>>>>>>> %s, %s',$dir,$name) );
	...
	public function filter_upload_dir( $dirs ) {
		...
		if ( ! defined( 'S3UPLOADS_DISABLE_REPLACE_UPLOAD_URL' ) || ! S3UPLOADS_DISABLE_REPLACE_UPLOAD_URL ) {
	...
	public function get_s3_url() {
		...
		return apply_filters( 's3uploads_bucket_url', 'https://' . $bucket . '.s3.amazonaws.com' . $path );
	}
	...
	public function blocked_uploads_header() {
		if ( current_user_can( $this->capability ) ) {
			...
			<p><?php printf( __( "Files can't be uploaded due to a billing issue with your s3Uploads account. <a href='%s'>Please resolve the issue</a> to resume uploading.", 's3uploads' ), esc_url( $this->admin->api_url( '/account/billing/' ) ) ); ?></p></div><?php
		} else {
			?>
			<div class="notice notice-error"><p><?php esc_html_e( "Files can't be uploaded due to a billing issue with your s3Uploads account.", 's3uploads' ); ?></p></div><?php
		}
	}
	...
	public function cdn_disabled_header() {
		if ( current_user_can( $this->capability ) ) {
			...
			if ( get_current_screen()->id == 'media_page_s3uploads'
			     || get_current_screen()->id == 'settings_page_s3uploads-network'
			     || ( get_current_screen()->id == 'media' && get_current_screen()->action == 'add' ) ) {
				return;
			}
			?>
			<div class="notice notice-error">
			<p><?php printf( __( "Files can't be uploaded and your CDN is disabled due to a billing issue with your s3Uploads account. <a href='%s'>Please resolve the issue</a> to resume uploading. <a href='%s'>Already fixed?</a>", 's3uploads' ), esc_url( $this->admin->api_url( '/account/billing/' ) ), esc_url( $this->admin->settings_url( [ 'refresh' => 1 ] ) ) ); ?></p>
			</div><?php
		}
	}
	...
	public function block_uploads( $file ) {
		$file['error'] = esc_html__( "Files can't be uploaded due to a billing issue with your s3Uploads account.", 's3uploads' );
	...
	function block_rest_upload( $result, $server, $request ) {
		...
		if ( preg_match( '%/wp/v2/media/\d+/edit%', $request->get_route() ) ) {
			$result = new WP_Error(
				'rest_cant_upload',
				__( "Files can't be uploaded due to a billing issue with your s3Uploads account.", 's3uploads' ),
				[ 'status' => 403 ]
			);
		}
	...
	public function filter_editors( $editors ) {
		...
		array_unshift( $editors, 'S3Uploads_Image_Editor_Imagick' );
	 ...
	 *
	 * We don't want to use the default uploads folder location, as with s3Uploads this is
