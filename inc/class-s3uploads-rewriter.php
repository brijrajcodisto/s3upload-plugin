...
class S3Uploads_Rewriter {
	...
	function __construct( $uploads_url, $replacements, $cdn_url ) {
		...
		$this->replacements = array_unique(
			array_map( [ $this, 'protocolize_url' ],
				apply_filters( 's3uploads_replacement_urls', $replacements )
			)
		);
	...
	protected function rewrite_url( $matches ) {
		 ...
		 *
		 * @param  {string}  $cdn_url  The base url to replace the match with, like `https://xxxxx.s3uploads.cloud/`.
		 ...
		 * @since  1.0
		 * @hook   s3uploads_rewrite_url
		 *
		 */
		return apply_filters( 's3uploads_rewrite_url', $this->cdn_url, $matches[0] );
	}
}
