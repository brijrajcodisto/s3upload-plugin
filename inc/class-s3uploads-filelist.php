...
class S3Uploads_Filelist {
	...
	/**
	 * S3Uploads_Filelist constructor.
	 *
	...
	public function __construct( $root_path, $timeout = 25.0, $paths_left = [] ) {
		...
		$this->instance   = S3Uploads::get_instance();
	}
	...
	public function start() {
		...
		if ( empty( $this->paths_left ) ) {
			...
			$result = $wpdb->query( "TRUNCATE TABLE {$wpdb->base_prefix}s3uploads_files" );
			//Sometimes hosts don't give the DB user TRUNCATE permissions, so DELETE all if we have to.
			if ( false === $result ) {
				$wpdb->query( "DELETE FROM {$wpdb->base_prefix}s3uploads_files WHERE 1" );
			}
			...
			foreach ( $contents as $item ) {
				...
				if ( is_link( $item ) || $this->is_excluded( $item ) ) {
					...
					if ( is_readable( $item ) ) {
						...
						$file = null;
						error_log( sprintf( '[S3UPLOADS Filelist Error] %s could not be read for syncing', $item ) );
					}
	...
	protected function is_excluded( $path ) {
		/**
		 * Filters the built in list of file/directory exclusions that should not be synced to the s3Uploads cloud. Be specific it's a simple strpos() search for the strings.
		 *
		 ...
		 * @since  1.0
		 * @hook   s3uploads_sync_exclusions
		 *
		 */
		$exclusions = apply_filters( 's3uploads_sync_exclusions', $this->exclusions );
	...
	protected function flush_to_db() {
		...
		if ( count( $this->file_list ) ) {
			...
			$query = "INSERT INTO {$wpdb->base_prefix}s3uploads_files (file, size, modified, type, errors) VALUES ";
			$query .= implode( ",\n", $values );
