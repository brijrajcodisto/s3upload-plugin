jQuery(document).ready(function ($) {
  ...
  var buildFilelist = function (remaining_dirs, nonce = '') {
    ...
    $.post(ajaxurl + '?action=s3uploads-filelist', data, function (json) {
      if (json.success) {
  ...
  var fetchRemoteFilelist = function (next_token, nonce = '') {
    ...
    $.post(ajaxurl + '?action=s3uploads-remote-filelist', data, function (json) {
      if (json.success) {
  ...
  var syncFilelist = function (nonce = '') {
    ...
    iupAjaxCall = $.post(ajaxurl + '?action=s3uploads-sync', data, function (json) {
      iupLoopErrors = 0;
      if (json.success) {
  ...
  var getSyncStatus = function () {
    ...
    $.get(ajaxurl + '?action=s3uploads-status', function (json) {
      if (json.success) {
        $('#iup-progress-size').text(json.data.remaining_size);
        $('#iup-progress-files').text(json.data.remaining_files);
        $('#iup-upload-progress').show();
  ...
  var deleteFiles = function () {
    ...
    $.post(ajaxurl + '?action=s3uploads-delete', {'nonce': iup_data.nonce.delete}, function (json) {
      if (json.success) {
        //$('.iup-progress-pcnt').text(json.data.pcnt_complete);
  ...
  var downloadFiles = function (nonce = '') {
    ...
    $.post(ajaxurl + '?action=s3uploads-download', data, function (json) {
      iupLoopErrors = 0;
      if (json.success) {
        //$('.iup-progress-pcnt').text(json.data.pcnt_complete);
  ...
  $('#iup-collapse-errors').on('show.bs.collapse', function () {
    ...
    $.get(ajaxurl + '?action=s3uploads-sync-errors', function (json) {
      if (json.success) {
  ...
  $('#iup-resync-button').on('click', function (e) {
    ...
    $.post(ajaxurl + '?action=s3uploads-reset-errors', {foo: "bar"}, function (json) {
      if (json.success) {
        $('.modal').modal('hide');
        $('#upload-modal').modal('show');
        return true;
  ...
  $('#iup-enable-button').on('click', function () {
    ...
    $('#iup-enable-button').hide();
    $('#iup-enable-spinner').removeClass('text-hide');
    $.post(ajaxurl + '?action=s3uploads-toggle', {'enabled': true, 'nonce': iup_data.nonce.toggle}, function (json) {
      if (json.success) {
        location.reload();
        return true;
      }
