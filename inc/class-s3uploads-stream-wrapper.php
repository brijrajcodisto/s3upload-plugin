...
use BaghelSoft\S3Uploads\Aws\S3\S3ClientInterface;
...
use BaghelSoft\S3Uploads\GuzzleHttp\Psr7\CachingStream;
use BaghelSoft\S3Uploads\Psr\Http\Message\StreamInterface;
use BaghelSoft\S3Uploads\Aws;
 ...
 *   operation
 */
class S3Uploads_Stream_Wrapper {
	...
	private function validate( $path, $mode ) {
		...
		if ( $mode == 'x' &&
			...
			$errors[] = "{$path} already exists on s3Uploads";
		}
	...
	private function debug( $action, $key ) {
		if ( defined( 'S3UPLOADS_SW_DEBUG' ) && S3UPLOADS_SW_DEBUG ) {
			...
			$pattern = '/' . preg_quote( WP_PLUGIN_DIR, '/' ) . '\/(?:(?!s3uploads)([^\/]+))\//';
			...
			$log = "[S3UPLOADS Stream Debug] $action $key";
			if ( defined( 'S3UPLOADS_SW_DEBUG_BACKTRACE' ) && S3UPLOADS_SW_DEBUG_BACKTRACE ) {
	...
	private function debug_cache( $action, $key ) {
		if ( defined( 'S3UPLOADS_SW_DEBUG_CACHE' ) && S3UPLOADS_SW_DEBUG_CACHE ) {
			$log = "[S3UPLOADS Stream Cache] $action $key";
			error_log( $log );
		}
	}
	...
	private function cacheObjectSet( $key, $body ) {
		...
		if ( strlen( $body ) > S3UPLOADS_STREAM_CACHE_MAX_BYTES ) {
			unset( $body );
	...
	public function stream_flush() {
		...
		if ( defined( 'S3UPLOADS_HTTP_EXPIRES' ) ) {
			$params['Expires'] = S3UPLOADS_HTTP_EXPIRES;
		}
		// Cache-Control:
		if ( defined( 'S3UPLOADS_HTTP_CACHE_CONTROL' ) ) {
			if ( is_numeric( S3UPLOADS_HTTP_CACHE_CONTROL ) ) {
				$params['CacheControl'] = 'max-age=' . S3UPLOADS_HTTP_CACHE_CONTROL;
			} else {
				$params['CacheControl'] = S3UPLOADS_HTTP_CACHE_CONTROL;
			}
		}
		 ...
		 * @since  1.0
		 * @hook   s3uploads_putObject_params
		 *
		 */
		$params = apply_filters( 's3uploads_putObject_params', $params );
		...
		return $this->boolCall( function () use ( $params ) {
			 ...
			 *
			 * @since 1.0
			 * @hook  s3uploads_putObject
			 *
			 */
			do_action( 's3uploads_putObject', $params, $bool );
	...
	public function dir_opendir( $path, $options ) {
		...
		$this->objectIterator = \BaghelSoft\S3Uploads\Aws\flatmap(
	...
	public function rename( $path_from, $path_to ) {
		...
		if ( ! $partsFrom['Key'] || ! $partsTo['Key'] ) {
			return $this->triggerError( 'The s3Uploads stream wrapper only '
