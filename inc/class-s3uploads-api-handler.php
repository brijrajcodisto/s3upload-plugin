...
class S3Uploads_Api_Handler {
	...
	public function __construct() {
		...
		if ( defined( 'S3UPLOADS_CUSTOM_API_SERVER' ) ) {
			$this->server_root = trailingslashit( S3UPLOADS_CUSTOM_API_SERVER );
		}
		...
		if ( is_main_site() ) {
			if ( ! wp_next_scheduled( 's3uploads_sync' ) ) {
				wp_schedule_event( time(), 'daily', 's3uploads_sync' );
			}

			add_action( 's3uploads_sync', [ $this, 'get_site_data' ] );
			add_action( 'wp_ajax_nopriv_s3uploads-refresh', [ &$this, 'remote_refresh' ] );
		}
	}

	/**
	 *
	 * @return S3Uploads_Api_Handler
	 */
	public static function get_instance() {

		if ( ! self::$instance ) {
			self::$instance = new S3Uploads_Api_Handler();
		}
	 ...
	 *
	 * Define S3UPLOADS_SITE_URL to override or make static the url it should show as
	...
	public function network_site_url() {
		return defined( 'S3UPLOADS_SITE_URL' ) ? S3UPLOADS_SITE_URL : network_site_url();
	}

	/**
	 * Returns the canonical home_url that should be used for the site on the site.
	 *
	 * Define S3UPLOADS_HUB_HOME_URL to override or make static the url it should show as
	...
	public function network_home_url() {
		if ( defined( 'S3UPLOADS_HOME_URL' ) ) {
			return S3UPLOADS_HOME_URL;
		} else {
	...
	public function call( $remote_path, $data = [], $method = 'GET', $options = [] ) {
		...
		$options = wp_parse_args(
			...
			[
				'timeout'    => 25,
				'user-agent' => 's3Uploads/' . S3UPLOADS_VERSION . ' (+' . network_site_url() . ')',
				'headers'    => [
		...
		if ( defined( 'S3UPLOADS_API_DEBUG' ) && S3UPLOADS_API_DEBUG ) {
			$log = '[S3UPLOADS API call] %s | %s: %s (%s)';
			if ( defined( 'S3UPLOADS_API_DEBUG_ALL' ) && S3UPLOADS_API_DEBUG_ALL ) {
				$log .= "\nRequest options: %s\nResponse: %s";
			}
	...
	public function remote_refresh( $urls ) {
		...
		if ( defined( 'S3UPLOADS_API_DEBUG' ) && S3UPLOADS_API_DEBUG ) {
			$log = '[S3UPLOADS API remote call] %s | %s';
